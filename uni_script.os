// UNI_SCRIPT - универсальный скрипт на языке 1С//	VERSION: 2017.07.11.1
// Версия
// TO-DO
...
Процедура СоздатьДистрибутивИзХранилища()	
	...
	Попытка
		...
		ЗаписьZIP.Добавить(мКаталогВерсииПолный + "1cv8.efd", РежимСохраненияПутейZIP.НеСохранятьПути);
		ЗаписьZIP.Добавить(мКаталогВерсииПолный + "setup", РежимСохраненияПутейZIP.НеСохранятьПути);
		ЗаписьZIP.Добавить(мКаталогВерсииПолный + "setup.exe", РежимСохраненияПутейZIP.НеСохранятьПути);
		ЗаписьZIP.Записать();
	...
	Если мМассивВерсий.Количество() > 0 Тогда
		...
		Попытка
			...
			ЗаписьZIP.Добавить(мКаталогВерсииОбновление + "1cv8.efd", РежимСохраненияПутейZIP.НеСохранятьПути);
			ЗаписьZIP.Добавить(мКаталогВерсииОбновление + "setup", РежимСохраненияПутейZIP.НеСохранятьПути);
			ЗаписьZIP.Добавить(мКаталогВерсииОбновление + "setup.exe", РежимСохраненияПутейZIP.НеСохранятьПути);
			ЗаписьZIP.Записать();
	...
	Попытка
		...
		Для а = 1 по Области.ВысотаТаблицы Цикл
			...
			Иначе
				...
				Стр.ТекстHTML 	= Стр.ТекстHTML + "<br />";
