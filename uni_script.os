...
Процедура СформироватьФайлНастроекVanessaAutomation()
	...
	Если СписокПараметров.Количество() > 4 Тогда
		...
		// Дополнительный параметр "НастройкиОтчетаAllure"
		Если КоличествоДопПараметров <> 0 Тогда
			
			НастройкиОтчетаAllure = "
				| 	""ДелатьОтчетВФорматеАллюр"": true,
				|	""ОтчетAllure"": {
				|		""КаталогВыгрузкиAllure"": ""<ПутьВыгрузкиОтчетаAllure>"",
				...
				|		""ДанныеАллюрМеток"": []
				|	},";
			КаталогВыгрузкиAllure = СписокПараметров[СписокПараметров.Количество() - КоличествоДопПараметров];
