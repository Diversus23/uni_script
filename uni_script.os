...
//				- /
	...
	мПараметры.Добавить("/C ""-Scenario " + мСценарий + "-PathIB """"" + мБаза + """"" -ErrorFile """"" + мФайлОшибок + """"" -CloseAfterExecution True""");
	мПараметры.Добавить("/TESTMANAGER");
	//мПараметры = "-Scenario " + мСценарий + " -PathIB " + мБаза + " -ErrorFile " + мФайлОшибок + " -CloseAfterExecution True";
	//Лог.Информация("Параметры запуска """ + мПараметры + """");
	
	Попытка
		Конфигуратор.ВыполнитьКоманду(мПараметры);	    
	Исключение												   
