// BSLLS-off
// UNI_SCRIPT - универсальный скрипт на языке 1С VERSION: 2021.01.22
// Версия
// TO-DO
...
Функция ЗаменитьПоРегулярномуВыражению(ИсходнаяСтрока, РегулярноеВыражение, СтрокаЗамены)
	...
	ТекстHTML = РегВыражение.Заменить(ИсходнаяСтрока, СтрокаЗамены);
	Возврат ТекстHTML;
...
Процедура КопироватьНаFTP()	
	...
	Попытка
		FTPСоединение = Новый FtpСоединение(мСервер, 21, мИмяПользователя, мПароль);
	...
	Попытка
		...
		ШапкаРассылки	= "<!-- Шапка -->
						...
						| <p style=""margin:10px 0;padding:0;color:#202020;font-family:Helvetica;font-size:16px;line-height:150%;text-align:left"">
			 			| Сегодня вышло очередное обновление нашего решения ""Управление IT-отделом 8, редакция 3.1"" версии ";
		...
		Для Каждого СтрокаВерсия Из МассивВерсийИзНастроек Цикл
			...
			ШапкаРассылки = ШапкаРассылки + "<b><a href=""https://softonit.ru/company/news/upduit-" + СтрЗаменить(СтрокаВерсия, ".", "-") + "/"" target=""_blank"">" + Строка(СтрокаВерсия) + "</a></b>";
		...
		Если МассивВерсийДляОбновления.Количество() > 0 Тогда
			...
			Для Каждого СтрокаВерсия Из МассивВерсийДляОбновления Цикл				
				ПодвалРассылки = ПодвалРассылки + "<b><a href=""https://softonit.ru/company/news/upduit-" + СокрЛП(СтрЗаменить(СтрокаВерсия, ".", "-"))
		...
		ТекстHTML = ТекстHTML + "Скачать обновления вы можете на нашем <a href=""https://softonit.ru/personal/lk/"" target=""_blank"">сайте</a>
			...
			| Если на данной странице Вы не видите обновлений, хотя Вы являетесь обладателем действующей подписки, то напишите на наш e-mail, выслав свой логин и мы 
			| включим Вам возможность скачивать обновления.<br>
...
Процедура ПодготовитьФайлОписанияОбновления()
	...
	ПутьКАрхивуОбновления = ПутьКАрхивуОбновления + Строка(VERSION) + "\Обновление\it_" + СтрЗаменить(VERSION, ".", "_") + "_upd.zip";
	...
	СтрокаXML = СтрЗаменить(СтрокаXML, "SITE_ID_PROGRAM", SITE_ID_PROGRAM);
	СтрокаXML = СтрЗаменить(СтрокаXML, "FILE_SIZE", FILE_SIZE);
	СтрокаXML = СтрЗаменить(СтрокаXML, "DATEVERSION", DATEVERSION);
	СтрокаXML = СтрЗаменить(СтрокаXML, "VERSION_UPDATES", VERSION_UPDATES);
	...
	Для Индекс = 1 По ТекстовыйДокумент.КоличествоСтрок() Цикл
		...
		Если СтрНайти(Стр, "<!-- JOURNAL -->") > 0 Тогда
	...
	НовыйАрхив = Новый ЗаписьZipФайла(ПутьКФайлуZip, , , МетодСжатияZIP.Сжатие, УровеньСжатияZIP.Максимальный, МетодШифрованияZIP.Zip20);
...
//				- Section
	...
	Если НЕ ЗначениеЗаполнено(ИДКаталога) Тогда
		Лог.Ошибка("Не заполнен параметр ""/SITE_ID_CATALOG"" - ID каталога на сайте в файле настроек");
		КодВозврата = 85;
		Возврат;
	КонецЕсли;	
...
Процедура ОпубликоватьНовости()
	...
	НовыйАрхив = Новый ЗаписьZipФайла(ИмяВременногоФайла, "",  "", МетодСжатияZIP.Сжатие, УровеньСжатияZIP.Максимальный, МетодШифрованияZIP.Zip20);
...
Процедура ПолучитьОписаниеОбновленияИзChangeLog()
	...
	// Меняем для ul класс если найдено <h4 class=""updnew"">
	ТекстHTML = ЗаменитьПоРегулярномуВыражению(ТекстHTML, "\<h4\>(Добавлено|Новый функционал|Added)\<\/h4\>\n<ul>", "<h4>$1</h4>" + Символы.ПС + "<ul class=""updnew"">");
	ТекстHTML = ЗаменитьПоРегулярномуВыражению(ТекстHTML, "\<h4\>(Изменения|Изменено|Changed)\<\/h4\>\n<ul>", "<h4>$1</h4>" + Символы.ПС + "<ul class=""updedt"">");
	ТекстHTML = ЗаменитьПоРегулярномуВыражению(ТекстHTML, "\<h4\>(Исправление ошибок|Замечания к обновлению|Fixed)\<\/h4\>\n<ul>", "<h4>$1</h4>" + Символы.ПС + "<ul class=""upderr"">");
	ТекстHTML = ЗаменитьПоРегулярномуВыражению(ТекстHTML, "\<h4\>(Внимание!|Новости|News|Attention!)\<\/h4\>\n<ul>", "<h4>$1</h4>" + Символы.ПС + "<ul class=""updnws"">");
