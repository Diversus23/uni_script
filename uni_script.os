...
//
//							  Шаблон строки параметров: /ParamsVA "<ВерсияВанессы>#<ТегиИсключения>|<ТегиИсключения>#<ТегиОтборов>|<ТегиОтборов>#<ТипОтладки>#<АдресОтладки>"
//							  Пример строки параметров: /ParamsVA "1.2.038.1#IgnoreOnCIMainBuild|FirstStart|Draft#Test1|Test2|Test3|#-http#http://localhost:1550"
...
Процедура СформироватьФайлНастроекVanessaAutomation()
	...
	СтрокаНастроекVanessa = "{
		...
		|    ""КаталогOutputAllureБазовый"": "".\\..\\build\\artifacts"",
		...
		|    ""ИмяФайлаЛогВыполненияСценариев"": ""$workspaceRoot\\vanessaonline_testing.log"",
	...
	Если СписокПараметров.Количество() = 0 Тогда
		ШаблонСтроки = "<ВерсияВанессы>#<ТегиИсключения>|<ТегиИсключения>#<ТегиОтборов>|<ТегиОтборов>#<ТипОтладки>#<АдресОтладки>";
	...
	СписокТеговИсключений 	= СтрРазделить(СписокПараметров[1], "|", Ложь);
	СписокТеговОтборов		= СтрРазделить(СписокПараметров[2], "|", Ложь);
	ТипОтладки				= СписокПараметров[3];
	АдресОтладки			= СписокПараметров[4];
	...
	ТекстовыйДокумент.Записать(ПутьСохранения + "VBParams.json", КодировкаТекста.UTF8);

	Лог.Информация("Файл VBParams.json УСПЕШНО сохранен по пути: " + ПутьСохранения);
