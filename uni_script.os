...
Процедура ВключитьОтладкуБазы()
	...
	НомерШага = 1;
	Сообщить("Шаг №" + НомерШага + " " + "Добавление параметров запуска.");
	...
	НомерШага = 2;
	Попытка
		Сообщить("Шаг №" + НомерШага + " " + "Запуск конфигуратора в режиме отладки.");
		Конфигуратор.ВыполнитьКоманду(мПараметрыЗапуска);
	Исключение
