...
Процедура ПодготовитьФайлОписанияОбновления()
	...
	///!!! Заменить на поиск файла по маске SITE_ID_PROGRAM + "_" + СтрЗаменить(VERSION, ".", "_") + "_upd.zip" в папке ПутьКАрхивуОбновления
...
Процедура ПодготовитьТекстРассылкиMDФайла()
	...
	Если НЕ ФайлСуществует(ФайлШапкиРассылки) Тогда
		Лог.Ошибка("Не существут файл параметр ""/UHeader""");
		КодВозврата = 54;
		Возврат;
	КонецЕсли;		
	...
	Если НЕ ФайлСуществует(ФайлТелаРассылки) Тогда
		Лог.Ошибка("Не существут файл параметр ""/UBody""");
		КодВозврата = 54;
		Возврат;
	КонецЕсли;	
	...
	Если НЕ ФайлСуществует(ФайлПодвалаРассылки) Тогда
		Лог.Ошибка("Не существут файл параметр ""/UFooter""");
		КодВозврата = 54;
		Возврат;
	КонецЕсли;
