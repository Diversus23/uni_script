...
//				- /Coverage41C признак включения анализа покрытия Coverage41C.
//				- /DebugURL адрес отладчика, например, http://127.0.0.1:1550/.
//				- /SrcFiles путь к выгруженным исходникам проекта ЕДТ.
...
Функция ЗаполнитьПараметры()
	...
	Парсер.ДобавитьИменованныйПараметр("/SrcFiles");
...
Процедура ЗапускVanessaAutomation()
	...
	Если ВРег(ЗапускCoverage41C) = "TRUE" Тогда
		
		// Первым делом запускаем сервер отладки на порту, который будет использовать клиент тестирования. Это нужно для успешного соединения Coverage41C с клиентом.
		ЗапуститьФайлОтладчика();

		// Дальше собираем данные для запуска анализа покрытия.
		СтрокаЗапускаCoverage   = "Coverage41C start -i DefAlias -u <АдресОтладки> -s <ПутьКИсходникам> -P <ПутьКПроектуЕДТ> -o <ПрефиксФайла>.xml";
		СтрокаОстановкиCoverage = "Coverage41C stop -i DefAlias -u <АдресОтладки>";
		СтрокаПроверкиCoverage	= "Coverage41C check -i DefAlias -u <АдресОтладки>";
		...
		СтрокаПроверкиCoverage  = СтрЗаменить(СтрокаПроверкиCoverage, "<АдресОтладки>", АдресОтладки);

		ПутьКИсходникам = Параметры["/SrcFiles"];
		Если Не ЗначениеЗаполнено(ПутьКИсходникам) Тогда
			Лог.Ошибка("Произошла ошибка, не заполнен параметр /SrcFiles");
			КодВозврата = 119;
			Возврат;
		КонецЕсли;
		СтрокаЗапускаCoverage = СтрЗаменить(СтрокаЗапускаCoverage, "<ПутьКИсходникам>", ПутьКИсходникам);
		...
		Попытка
			...
			Лог.Информация("Выполним проверку, что Coverage41C запустился и работает");
			ЗапуститьПриложение(СтрокаПроверкиCoverage, , Истина, КодВозврата);
		Исключение
	...
	Если ВРег(ЗапускCoverage41C) = "TRUE" Тогда
		Попытка
			...
			ЗапуститьПриложение(СтрокаОстановкиCoverage, , Ложь);
		Исключение
			Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			Лог.Ошибка(Ошибка);
			КодВозврата = 123;
...
Процедура ЗапуститьФайлОтладчика()
	...
	Попытка
		...
		Лог.Информация("Строка запуска: " + СтрокаЗапуска + "--port=" + ПортОтладки);
		ЗапуститьПриложение("""" + СтрокаЗапуска + """" + " " + "--port=" + ПортОтладки, , Ложь);
	Исключение
