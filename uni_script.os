// BSLLS-off
// UNI_SCRIPT - универсальный скрипт на языке 1С VERSION: 2023.04.27
// Версия
// TO-DO
...
// oscript "c:\Projects\uni_script\uni_script.os" /DeleteInfoBase /S alba\zmysupertest /DB_User sa /DB_Password *** /DB_Name zmysupertest /DB_Server alba
Процедура УдалитьИнформационнуюБазу()
	...
	ИначеЕсли ЗначениеЗаполнено(Параметры["/S"]) Тогда
		...
		Если ЗначениеЗаполнено(Параметры["/DB_Type"]) Тогда 
			ТипСУБД = Параметры["/DB_Type"];
		Иначе
			ТипСУБД = "MSSQLServer";
		КонецЕсли;
		Если ЗначениеЗаполнено(Параметры["/DB_Server"]) Тогда 
			СерверСУБД = Параметры["/DB_Server"];
		Иначе
			СерверСУБД = "127.0.0.1";
		КонецЕсли;
		Лог.Информация("Сервер БД " + ТипСУБД + ": " + СерверСУБД);

		Если ЗначениеЗаполнено(Параметры["/DB_User"]) Тогда 
			ПользовательСУБД = Параметры["/DB_User"];
			Лог.Информация("ПользовательСУБД: " + ПользовательСУБД);
		Иначе
			ПользовательСУБД = "";
		КонецЕсли;		

		Если ЗначениеЗаполнено(Параметры["/DB_Password"]) Тогда 
			ПарольСУБД = Параметры["/DB_Password"];
			Лог.Информация("ПарольСУБД: ***");
		Иначе
			ПарольСУБД = "";
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Параметры["/DB_Name"]) Тогда 
			ИмяБД = Параметры["/DB_Name"];
		Иначе
			ИмяБД = ИмяБазы;
		КонецЕсли;
		Лог.Информация("Имя базы БД: " + ИмяБД);
				...
				Для Каждого Процесс из Процессы Цикл
					...
					Для Каждого База из Базы Цикл										
						Если ВРег(СокрЛП(База.Name)) = ВРег(СокрЛП(ИмяБазы)) Тогда						
							...
							Шаблон.Name 		= ИмяБазы;
							Шаблон.DBMS			= ТипСУБД;
							Шаблон.dbUser		= ПользовательСУБД;
							Шаблон.dbServerName = СерверСУБД;
							Шаблон.dbPassword 	= ПарольСУБД;
							Шаблон.dbName   	= ИмяБД;
							РабПроц.DropInfoBase(Шаблон, 2); 
