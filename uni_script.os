	...
	Попытка
		...
		Для а = 1 по Области.ВысотаТаблицы Цикл
			...
			Если Области.Область(а, 2, а, 2).Гиперссылка = Истина Тогда
				...
				НайденныйURLGIF = НастройкиURL.Получить(ИмяОбласти + "_gif");
				...
				Если НайденныйURLGIF <> Неопределено Тогда
					Если СтрНайти(НРег(НайденныйURLGIF), ".gif", НаправлениеПоиска.СКонца) > 0 Тогда
						ПоследнийСлеш	= СтрНайти(НайденныйURLGIF, "/", НаправлениеПоиска.СКонца);
						ИДВидео 		= Строка(Сред(НайденныйURLGIF, ПоследнийСлеш + 1));
						АдресФайла 		= НайденныйURLGIF;
					КонецЕсли;
				КонецЕсли;
			...
			Если НЕ ПустаяСтрока(Раздел) Тогда
				Если Раздел = "Внимание!" Тогда
					...
					Если Области.Область(Индекс, 2, Индекс, 2).Гиперссылка = Истина Тогда
						...
						ИмяСсылкиGif	= ИмяСсылки + "_gif";
						
						НайденныйURL	= Неопределено;
						НайденныйURLGIF = Неопределено;
						Для Каждого Элемент Из СписокСсылок Цикл
							Если Элемент.Ключ = ИмяСсылки Тогда
								НайденныйURL 	= Элемент.Значение.Ссылка;
							КонецЕсли;

							Если Элемент.Ключ = ИмяСсылкиGif Тогда
								НайденныйURLGIF	= Элемент.Значение.Ссылка;
							КонецЕсли;
						...
						Если СтрНайти(НайденныйURL, "youtu") > 0 Тогда
						...
						Если НайденныйURLGIF <> Неопределено Тогда
							ПараметрыОбработки = Новый Структура("link_video, link_preview");
							ПараметрыОбработки.link_video = НайденныйURLGIF;
							ПараметрыОбработки.link_preview = "https://softonit.ru/upload/news/" + Сред(НайденныйURLGIF, СтрНайти(НайденныйURLGIF, "/", НаправлениеПоиска.СКонца) + 1) + ".jpg";
							УстановитьПараметрыHTML(СекцияПерьвюВидео, ПараметрыОбработки);
							СтрокаПревью = СтрокаПревью + СекцияПерьвюВидео;
						КонецЕсли;
