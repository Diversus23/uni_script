		...
		Для Каждого Стр Из МассивОшибок Цикл
			...
			Если ВРег(ПолучитьУпавшиеТесты) = "TRUE" Тогда
				...
				Если ПозицияПутиНачало <> 0 Тогда
					...
					СтрокиДиректорииФичи = СтрРазделить(НайденныйПутьКФиче, "\", Ложь);
					ИмяФичи	= СтрокиДиректорииФичи[СтрокиДиректорииФичи.Количество() - 1];
					ТекстДиректорий	= ТекстДиректорий + ИмяФичи + Символы.ПС;
