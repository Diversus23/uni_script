...
//				- /PrefixFile префикс создаваемого файла покрытия.
//	
...
//		
//		/StartDebugBase - Запускает информационную базу в режиме отладки.
//				- /F <путь> (тип базы - файловая)
//				- /TypeDebug - тип отладки http, tcp.
//				- /DebugURL - адрес отладчика.
//				- /AutoLinkDebug - признак включения автоматического подключения сеансов отладки, задается значением True, False.
//				- /Settings - путь к файлу settings.ini.
//				- /Section - раздел файла settings.ini с настройками.
//
...
Процедура ВыполнитьКоманду()
	...
	ИначеЕсли ВРегИмяКоманды = ВРег("/StartDebugBase") Тогда
		ВключитьОтладкуБазы();
...
Процедура ПодготовитьТекстРассылки()
	...
	Попытка
		...
		// Подготовка шапки.
		ШапкаРассылки	= "<!-- Шапка -->
						| <h2 style=""text-align: center;"">Пришло время для установки обновлений!</h2>
		 				| <br>
		...
		Для Каждого СтрокаВерсия Из МассивВерсийИзНастроек Цикл
			//<b><a href="https://softonit.ru/company/news/upduit-3-1-8-4/" target="_blank">3.1.8.4</a></b> и
			СчетчикСтрок  = СчетчикСтрок + 1;
			ШапкаРассылки = ШапкаРассылки + "<b><a href=""https://softonit.ru/company/news/upd" + ИдентефикаторПрограммы +"-" + СтрЗаменить(СтрокаВерсия, ".", "-") + "/"" target=""_blank"">" + Строка(СтрокаВерсия) + "</a></b>";
		...
		Если МассивВерсийДляОбновления.Количество() > 0 Тогда
			...
			Для Каждого СтрокаВерсия Из МассивВерсийДляОбновления Цикл				
				ПодвалРассылки = ПодвалРассылки + "<b><a href=""https://softonit.ru/company/news/upd" + ИдентефикаторПрограммы +"-" + СокрЛП(СтрЗаменить(СтрокаВерсия, ".", "-"))
...
Процедура ЗапускVanessaAutomation()
	...
	// Сборка параметров для запуска анализа покрытия тестами.
	// Пример строки запуска Coverage: Coverage41C start -i DefAlias -u http://127.0.0.1:1550 -s $CI_PROJECT_DIR\$PROJECT_NAME\src -P $CI_PROJECT_DIR -o $CI_PROJECT_DIR\$PROJECT_NAME_genericCoverage.xml
	ЗапускCoverage41C = Параметры["/Coverage41C"];
	Если ЗапускCoverage41C = ВРег("True") Тогда
		Лог.Информация("Формирование строки запуска Coverage41C");
		СтрокаЗапускаCoverage   = "Coverage41C start -i DefAlias -u <АдресОтладки> -s <ПутьКИсходникамПроекта> -P <ПутьКПроекту> -o <ПутьКПроекту>\<ПрефиксФайлаПокрытия>_genericCoverage.xml";
		СтрокаПроверкиСтатуса   = "Coverage41C start check -i DefAlias -u <АдресОтладки>";
		...
		Если Не ЗначениеЗаполнено(АдресОтладки) Тогда
			Лог.Ошибка("Не заполнен параметр <DebugURL>");
			КодВозврата = 117;
		КонецЕсли;
		СтрокаЗапускаCoverage   = СтрЗаменить(СтрокаЗапускаCoverage, "<АдресОтладки>", АдресОтладки);
		СтрокаПроверкиСтатуса   = СтрЗаменить(СтрокаПроверкиСтатуса, "<АдресОтладки>", АдресОтладки);
		СтрокаОстановкиCoverage = СтрЗаменить(СтрокаПроверкиСтатуса, "<АдресОтладки>", АдресОтладки);

		ПутьКИсходникам = Параметры["/FilesSrc"];
		Если Не ЗначениеЗаполнено(ПутьКИсходникам) Тогда
			Лог.Ошибка("Не заполнен параметр <FilesSrc>");
			КодВозврата = 118;
		КонецЕсли;
		СтрокаЗапускаCoverage = СтрЗаменить(СтрокаЗапускаCoverage, "<ПутьКИсходникамПроекта>", ПутьКИсходникам);

		ПутьКПроекту = Параметры["/Project"];
		Если Не ЗначениеЗаполнено(ПутьКПроекту) Тогда
			Лог.Ошибка("Не заполнен параметр <Project>");
			КодВозврата = 119;
		КонецЕсли;
		СтрокаЗапускаCoverage = СтрЗаменить(СтрокаЗапускаCoverage, "<ПутьКПроекту>", ПутьКПроекту);

		ПрефиксФайла = Параметры["/PrefixFile"];
		Если Не ЗначениеЗаполнено(ПрефиксФайла) Тогда
			Лог.Ошибка("Не заполнен параметр <PrefixFile>");
			КодВозврата = 120;
		КонецЕсли;
		СтрокаЗапускаCoverage = СтрЗаменить(СтрокаЗапускаCoverage, "<ПрефиксФайлаПокрытия>", ПрефиксФайла);

		Лог.Информация("Итоговая строка запуска Coverage41C: " + СтрокаЗапускаCoverage);
	...
	Попытка
		// Выполняем запуск утилиты формирования покрытия перед запуском тестирования без ожидания завершения.
		Если ЗапускCoverage41C = ВРег("True") Тогда
			Попытка
				Лог.Информация("Запуск Coverage41C");
				ЗапуститьПриложение(СтрокаЗапускаCoverage, , Истина, КодВозврата);
			Исключение
				Лог.Ошибка("Произошла ошибка при запуске анализа покрытия");
				КодВозврата = 122;
			КонецПопытки;
			Лог.Информация("Анализ покрытия успешно запущен");
		КонецЕсли; 
		ЗапуститьПриложение(СтрокаЗапуска, , Истина, КодВозврата);
	Исключение												   
		Лог.Ошибка("Произошла ошибка при тестировании конфигурации");
	...
	Если ЗапускCoverage41C = ВРег("True") Тогда
		Попытка
			Лог.Информация("Завершение анализа покрытия кода тестами");
			ЗапуститьПриложение(СтрокаОстановкиCoverage, , Истина, КодВозврата);
		Исключение
			Лог.Ошибка("Произошла ошибка при завершении анализа покрытия");
			КодВозврата = 121;
		КонецПопытки;
		Лог.Информация("Анализа покрытия кода тестами завершен");
	КонецЕсли;
...
КонецПроцедуры
Процедура ВключитьОтладкуБазы()
	
	КаталогБазы = Параметры["/F"];
	Если Не ЗначениеЗаполнено(КаталогБазы) Тогда
		Лог.Ошибка("Не заполнен параметр ""/F <Путь>"" для файловой базы.");
		КодВозврата = 105;
	КонецЕсли;

	ТипОтладки = Параметры["/TypeDebug"];
	Если Не ЗначениеЗаполнено(ТипОтладки) Тогда
		Лог.Ошибка("Не заполнен параметр ""/TypeDebug"", который отвечает за режим запуска отладки.");
		КодВозврата = 106;
	КонецЕсли;

	АдресОтладчика = Параметры["/DebugURL"];
	Если Не ЗначениеЗаполнено(АдресОтладчика) Тогда
		Лог.Ошибка("Не заполнен параметр ""/DebugURL"", который отвечает за адрес отладчика.");
		КодВозврата = 107;
	КонецЕсли;

	АвтоПодключение = Параметры["/AutoLinkDebug"];
	КлючАвтоПодключения = "";
	Если НРег(АвтоПодключение) = "true" Тогда
		КлючАвтоПодключения = "-attach";
	КонецЕсли;
	...
	НомерШага = 1;
	Сообщить("Шаг №" + НомерШага + " " + "Формирование параметров запуска.");

	мПараметрыЗапуска = Конфигуратор.ПолучитьПараметрыЗапуска();
	мПараметрыЗапуска[0] = "Enterprise";
	мПараметрыЗапуска.Добавить("/debug" + " " + ТипОтладки + " " + КлючАвтоПодключения);
	мПараметрыЗапуска.Добавить("/debuggerURL" + " " + АдресОтладчика);

	СтрокаЗапуска = "";
	Для Каждого Параметр Из мПараметрыЗапуска Цикл
		СтрокаЗапуска = СтрокаЗапуска + " " + Параметр;
	КонецЦикла;
	Сообщить("Итоговая строка запуска:" + " " + СтрокаЗапуска + " " + ПутьКПлатформе);
	
	НомерШага = 2;
	Попытка
		Сообщить("Шаг №" + НомерШага + " " + "Запуск конфигурации в режиме отладки.");
		ЗапуститьПриложение("""" + ПутьКПлатформе + """" + СтрокаЗапуска, , Ложь, КодВозврата);
	Исключение
	...
	НомерШага = 3;
	Попытка
		Сообщить("Шаг №" + НомерШага + " " + "Принудительный запуск dbgs.exe для включения отладки.");
		ЗапуститьПриложение("""" + ПутьКПлатформе + """" + "dbgs.exe", , Ложь, КодВозврата);
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Лог.Ошибка(ТекстОшибки);
		КодВозврата = 110;
...
Процедура ЗапуститьФайлОтладчика()
	...
	НомерШага = 2;
	Попытка
		Лог.Информация("Шаг №" + НомерШага + " " + "Принудительный запуск dbgs.exe для включения отладки.");
		Лог.Информация("Строка запуска: " + СтрокаЗапуска + "--addr=" + АдресОтладчика + "--port=" + ПортОтладки);
		ЗапуститьПриложение("""" + СтрокаЗапуска + """" + "--addr=" + АдресОтладчика + "--port=" + ПортОтладки, , Ложь, КодВозврата);
	Исключение
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Лог.Ошибка(ТекстОшибки);
		КодВозврата = 116;
