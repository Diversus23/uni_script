...
Процедура юсВыполнитьКоманду()
	...
	Если ВРегИмяКоманды = ВРег("/StartTest") Тогда 
		ЗапускТестовойСреды();
	КонецЕсли;
...
Процедура Создать_Install_edf()
	...
	Файл.ДобавитьСтроку("},");
	Файл.ДобавитьСтроку("{1,");
	Файл.ДобавитьСтроку("{366f16f3-e8bf-455a-a547-b990015c4a94,""Общие файлы"",00000000-0000-0000-0000-000000000000,""" + мКаталогВерсийОбновлений + "Общие файлы для дистрибутива"",""*.*"",1}");
	Файл.ДобавитьСтроку("},");
...
//				- /
	...
	мПараметры = Параметры["/Params"];
	Если НЕ ЗначениеЗаполнено(мПараметры) Тогда
		Лог.Ошибка("Не заполнен параметр ""/Params"" - список параметров для тестирования конфигурации");
		КодВозврата = 88;
		Возврат;
	КонецЕсли;

	мФайлОшибок = Параметры["/ErrorFile"];
	Если НЕ ЗначениеЗаполнено(мФайлОшибок) Тогда
		Лог.Ошибка("Не заполнен параметр ""/ErrorFile"" - файл ошибок тестирования (куда их сохранять)");
		КодВозврата = 89;
		Возврат;
	КонецЕсли;

	Лог.Информация("Параметры тестирования """ + мПараметры + """");
	
	Попытка
	    Конфигуратор.ЗапуститьВРежимеПредприятия(мПараметры, Неопределено, "/TESTMANAGER");
		
	Исключение												   
		Лог.Ошибка("Произошла ошибка при тестировании конфигурации " + Конфигуратор.ВыводКоманды());
		КодВозврата = 90;
		Возврат;
	КонецПопытки;
