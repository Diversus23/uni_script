...
Процедура ЗапускVanessaAutomation()
	...
	Попытка
		...
		Если ЗапускCoverage41C = ВРег("True") Тогда
			...
			ЗапуститьПриложение(СтрокаЗапускаCoverage, , Ложь, КодВозврата);
			Лог.Информация("Проверка статуса Coverage41C");
			ЗапуститьПриложение(СтрокаПроверкиСтатуса, , Ложь, КодВозврата);
		КонецЕсли; 
		ЗапуститьПриложение(СтрокаЗапуска, , Истина, КодВозврата);
	Исключение												   
		Лог.Ошибка("Произошла ошибка при тестировании конфигурации");
	...
	Если ЗапускCoverage41C = ВРег("True") Тогда
		...
		ЗапуститьПриложение(СтрокаОстановкиCoverage, , Ложь, КодВозврата);
	КонецЕсли;
