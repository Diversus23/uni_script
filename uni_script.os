// BSLLS-off
// UNI_SCRIPT - универсальный скрипт на языке 1С VERSION: 2022.09.19
// Версия
// TO-DO
...
//				- /Prefix префикс продукта, если на задан будет прочтен из файла настроек
...
Процедура СоздатьДистрибутивОтчетаИлиОбработки()
	...
	Если НЕ ЗначениеЗаполнено(мПрефикс) Тогда
		мПрефикс = Настройки.Получить("SITE_ID_PROGRAM");	
		Если НЕ ЗначениеЗаполнено(мПрефикс) Тогда		
			Лог.Ошибка("Не заполнен параметр ""/SITE_ID_PROGRAM"" - ID программы в файле настроек или параметр /Prefix!");
			КодВозврата = 84;
			Возврат;
		КонецЕсли;
	КонецЕсли;
