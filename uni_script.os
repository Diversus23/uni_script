...
Процедура СформироватьФайлНастроекVanessaAutomation()
	...
	Если СписокПараметров.Количество() > 4 Тогда
		...
		// Дополнительный списковых параметр "СписокТеговОтборов"
		СтрокаТеговОтбров = "
			|""СписокТеговОтбор"": [
			|		<ТегиОтборов>
			|	],";
		СписокТеговОтборов = СтрРазделить(СписокПараметров[СписокПараметров.Количество() - 1], "|", Ложь);
		...
		СтрокаПути 	 		= "$workspaceRoot\\vanessaonline_testing<ПрефиксИмениЛога>.log";
		ИндексПараметра		= СписокПараметров.Количество() - 1;
		ПараметрПрефикса	= СписокПараметров[ИндексПараметра];
		
		ОписаниеЧисла = Новый ОписаниеТипов("Число");
		Если ОписаниеЧисла.ПривестиЗначение(ПараметрПрефикса) <> 0 Тогда
			ПараметрПрефикса = Число(ПараметрПрефикса);
		Иначе
			Лог.Ошибка("Произошла ошибка, в качестве префикса файла лога не было передано ЧИСЛО.");
			КодВозврата = 131;
			Возврат;
		КонецЕсли;
	...
	Иначе
		// Иначе очищаем итоговую строку настроек от вставленных дополнительных параметров, если доп.параметры не были переданы.
