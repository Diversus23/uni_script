// BSLLS-off
// UNI_SCRIPT - универсальный скрипт на языке 1С VERSION: 2023.05.04
// Версия
// TO-DO
...
//
//		+ /DumpIBWithoutLock		Резервная копия без блокировкой сеансов, работает аналогично DumpIB, но не блокирует сеансы
// 	
...
//
//		+ /RestoreIBWithoutLock		Восстановление резервеной копии в ИБ без блокировкой сеансов, работает аналогично RestoreIB, но не блокирует сеансы
//
...
//
//		+ /CreateInfoBase - создание пустой информационной базы в каталоге или на сервере
//				- /F <путь> (тип базы - файловая)
//				- /S <адрес> (тип базы - серверная, <Имя компьютера, работающего сервером приложений>\<Ссылочное имя информационной базы, известное в рамках сервера 1С:Предприятия 8>)
//
//		+ /DeleteInfoBase - удаление информационной базы в каталоге или на сервере
//				- /F <путь> (тип базы - файловая)
//				- /S <адрес> (тип базы - серверная, <Имя компьютера, работающего сервером приложений>\<Ссылочное имя информационной базы, известное в рамках сервера 1С:Предприятия 8>)
//				- /DeleteMode (число 0 - при удалении информационной базы базу данных не удалять; 1 - при удалении информационной базы удалить базу данных (по умолчанию); 2 - при удалении информационной базы очистить базу данных.)
//
...
//				- /P (пароль администратора ИБ)
//				- /UPath куда сохранить
//				- /Catalog путь откуда брать составные чати общего шаблона HTML
//				- /Maket имя общего макета для создания файла с новостями, если не определено, то равен "ОписаниеИзмененийСистемы"
//				- /Settings путь к файлу settings.ini
//				- /Section раздел файла settings.ini с настройками
//
...
//				- /Section раздел файла settings.ini с настройками
//
//		/DumpExternalDataProcessorOrReportToFiles - Выполняет выгрузку обработок и отчетов в папку в файлы XML
//				- /FileName - имя файла обработки или отчета
//				- /Catalog <путь> - Путь куда выгрузить обработку/отчет
//
//		/VanessaAutomationTest - выполняет запуск тестовой среды VanessaAutomation и тестов
//				- /F <путь> (тип базы - файловая)
//				- /S <адрес> (тип базы - серверная, <Имя компьютера, работающего сервером приложений>\<Ссылочное имя информационной базы, известное в рамках сервера 1С:Предприятия 8>)
//				- /N (логин администратора ИБ, если не задан, то без логина)
//				- /P (пароль администратора ИБ)
...
Функция ЗаполнитьПараметры()
	...
	Парсер.ДобавитьИменованныйПараметр("/UseRegl");
	Парсер.ДобавитьИменованныйПараметр("/DB_Type");
	Парсер.ДобавитьИменованныйПараметр("/DB_Server");
	Парсер.ДобавитьИменованныйПараметр("/DB_User");
	Парсер.ДобавитьИменованныйПараметр("/DB_Password");
	Парсер.ДобавитьИменованныйПараметр("/DB_Name");
	Парсер.ДобавитьИменованныйПараметр("/Template");
	Парсер.ДобавитьИменованныйПараметр("/BaseNameList");
	Парсер.ДобавитьИменованныйПараметр("/DeleteMode");
	Парсер.ДобавитьИменованныйПараметр("/VAParams");
	Парсер.ДобавитьИменованныйПараметр("/WorkspaceRoot");
...
Процедура ВыполнитьКоманду()
	...
	ИначеЕсли ВРегИмяКоманды = "/RESTOREIBWITHOUTLOCK" Тогда		
		Если ЗначениеЗаполнено(ПутьКБазеСерверной) Тогда
			ПрерватьСеансыПользователей();
			ЗагрузитьИБИзФайла();
		Иначе
	...
	ИначеЕсли ВРегИмяКоманды = "/DUMPIBWITHOUTLOCK" Тогда
		Если ЗначениеЗаполнено(ПутьКБазеСерверной) Тогда
			ПрерватьСеансыПользователей();
			ВыгрузитьИБВФайл();			
			юсУдалитьФайлы();
	...
	ИначеЕсли ВРегИмяКоманды = ВРег("/DeleteInfoBase") Тогда 
	...
	ИначеЕсли ВРегИмяКоманды = ВРег("/DumpExternalDataProcessorOrReportToFiles") Тогда
		ВыгрузитьВнешнийОтчетИлиОбработкуВФайлы();

	ИначеЕсли ВРегИмяКоманды = ВРег("/VanessaAutomationTest") Тогда
		ВыполнитьТестированиеVanessaAutomation();
...
Процедура юсУдалитьФайлы()
	...
	Если ЗначениеЗаполнено(Параметры["/UCount"]) Тогда
...
Процедура ПолучитьОписаниеОбновленияИзМакета(ФайлHTML = Неопределено)
	...
	Попытка
		...
		МетаданныеМодуля = Соединение.Метаданные.ОбщиеМодули.Найти("ОбновлениеИнформационнойБазыКлиентПереопределяемый");
		Если МетаданныеМодуля <> Неопределено Тогда
			Лог.Информация("Получаем ссылки из общего модуля - <ОбновлениеИнформационнойБазыКлиентПереопределяемый>");
			СписокСсылок = Соединение.ОбновлениеИнформационнойБазыКлиентПереопределяемый.СтруктураОписанияОбновлений();
		Иначе
			Лог.Информация("Не удалось получить ссылки из общего модуля, поэтому получаем ссылки из файла settings.ini");
			СписокСсылок = ПрочитатьСекциюINIФайла(Settings, SectionURL);
		КонецЕсли;
		...
		Для а = 1 по Области.ВысотаТаблицы Цикл
			...
			Если Области.Область(а, 2, а, 2).Гиперссылка = Истина Тогда
				ИмяСсылки = Области.Область(а, 2, а, 2).Имя;

				НайденныйURL = Неопределено;
				Для Каждого Элемент Из СписокСсылок Цикл
					Если Элемент.Ключ = ИмяСсылки Тогда
						НайденныйURL = ?(ТипЗнч(СписокСсылок) = Тип("Соответствие"), Элемент.Значение, Элемент.Значение.Ссылка);;
					КонецЕсли;
				КонецЦикла;

				Если НайденныйURL = Неопределено Тогда
					Лог.Ошибка("В общем модуле ""ОбновлениеИнформационнойБазыКлиентПереопределяемый"" не указана гиперссылка для области <" + ИмяСсылки + ">");
					КодВозврата = 56;
				КонецЕсли;
	...
	// Очищаем соединения
	Попытка
		ОсвободитьОбъект(МетаданныеМодуля);		
	Исключение
	КонецПопытки;

	Попытка
		ОсвободитьОбъект(СписокСсылок);
	Исключение
	КонецПопытки;	
...
Процедура ПолучитьКартинкиДляРассылки()
	...
	Попытка
		...
		Макет 			= Соединение.ПолучитьОбщийМакет(ИмяМакета);
		Попытка
			Лог.Информация("Попытаемся получить ссылки из общего модуля - <ОбновлениеИнформационнойБазыКлиентПереопределяемый>");
			СписокСсылок 	= Соединение.ОбновлениеИнформационнойБазыКлиентПереопределяемый.СтруктураОписанияОбновлений();
		Исключение
			Лог.Информация("Не удалось получить ссылки из общего модуля, поэтому получаем ссылки из файла settings.ini");
			СписокСсылок 	= ПрочитатьСекциюINIФайла(Settings, SectionURL);
		...
		Для а = 1 по Области.ВысотаТаблицы Цикл
			...
			Если Области.Область(а, 2, а, 2).Гиперссылка = Истина Тогда
				...
				Если НайденныйURL = Неопределено Тогда
					...
					Лог.Ошибка("В общем модуле <ОбновлениеИнформационнойБазыКлиентПереопределяемый> не указана ссылка для области " + "<" + ИмяСсылки + ">");
					КодВозврата = 148;
					Возврат;
				КонецЕсли;				
				...
				Если СтрНайти(НРег(НайденныйURL), ".gif", НаправлениеПоиска.СКонца) > 0 Тогда
					ПоследнийСлеш	= СтрНайти(НайденныйURL, "/", НаправлениеПоиска.СКонца);
					ИДВидео 		= Строка(Сред(НайденныйURL, ПоследнийСлеш + 1));
					АдресФайла 		= НайденныйURL;					
				КонецЕсли;
				...
				Если Не ПустаяСтрока(ИДВидео) Тогда
					...
					НастройкиСоединения.Вставить("ЗащищенноеСоединение", Ложь);
...
//				- /Catalog <путь> (Путь к каталогу конфигурации)
	...
	// Шаг 2. Обновление загруженной конфигурации базы данных
	// ПараметрыЗапуска2 = Конфигуратор.ПолучитьПараметрыЗапуска();
	// ПараметрыЗапуска2.Добавить("/UpdateDBCfg");
	// Если ЗначениеЗаполнено(Параметры["/S"]) Тогда
	// 	Конфигуратор.УстановитьКонтекст("/S" + Параметры["/S"], "", "");		
	// Иначе
	// 	Конфигуратор.УстановитьКонтекст("/F" + Параметры["/F"], "", "");
	// КонецЕсли;		
	// Если ЗначениеЗаполнено(Параметры["/S"]) Тогда
	// 	ПараметрыЗапуска2.Добавить("-Server");
	// КонецЕсли;	
...
// oscript "c:\Projects\uni_script\uni_script.os" /CreateInfoBase /F "c:\Temp\1\"
// oscript "c:\Projects\uni_script\uni_script.os" /CreateInfoBase /S "192.168.1.20\temp12345" /DB_Server "192.168.1.20" /DB_User sa /DB_Password ****
Процедура СоздатьИнформационнуюБазу()
	...
	Если ЗначениеЗаполнено(Параметры["/F"]) Тогда
		...
		ПутьКШаблону = "";
		Если ЗначениеЗаполнено(Параметры["/Template"]) Тогда 
			ПутьКШаблону = Параметры["/Template"];
			Лог.Информация("Шаблон создаваемой ИБ: " + ПутьКШаблону);
		КонецЕсли;

		ИмяБазыВСписке = "";
		Если ЗначениеЗаполнено(Параметры["/BaseNameList"]) Тогда 
			ИмяБазыВСписке = Параметры["/BaseNameList"];
			Лог.Информация("Имя в списке баз пользователя: " + ИмяБазыВСписке);
		КонецЕсли;
		
		Попытка
			Конфигуратор.СоздатьФайловуюБазу(Каталог, ПутьКШаблону, ИмяБазыВСписке);
			Лог.Информация("Создание ИБ завершено");
		Исключение												   
			Лог.Ошибка("Произошла ошибка при создании файловой ИБ " + Конфигуратор.ВыводКоманды());
			КодВозврата = 70;
			Возврат;
		КонецПопытки;		

	ИначеЕсли ЗначениеЗаполнено(Параметры["/S"]) Тогда

		// Создаем серверную базу
		Массив = СтрРазделить(Параметры["/S"], "\");
		Если Массив.Количество() <> 2 Тогда
			Лог.Ошибка("Не верно передано сервер и имя базы. Необходим формат: server\base1c передано <" + Параметры["/S"] + ">");
			КодВозврата = 70;
			Возврат;
		КонецЕсли;
		...
		ПараметрыБазы1С.Вставить("Сервер1С", Массив[0]);
		Лог.Информация("Сервер 1С: " + ПараметрыБазы1С.Сервер1С);
		ПараметрыБазы1С.Вставить("ИмяИБ", Массив[1]);
		Лог.Информация("Имя базы 1С: " + ПараметрыБазы1С.ИмяИБ);
		ПараметрыБазы1С.Вставить("РазрешитьРегЗадания", Ложь);
		Если ЗначениеЗаполнено(Параметры["/UseRegl"]) Тогда
			ПараметрыБазы1С.Вставить("РазрешитьРегЗадания", Истина);
			Лог.Информация("РазрешитьРегЗадания: Истина");
		КонецЕсли;
		ПараметрыБазы1С.Вставить("РазрешитьВыдачуЛицензий", Истина);
		
		// Параметры СУБД 
		ПараметрыСУБД = Новый Структура();
		Если ЗначениеЗаполнено(Параметры["/DB_Type"]) Тогда 
			ПараметрыСУБД.Вставить("ТипСУБД", Параметры["/DB_Type"]);
		Иначе
		...
		Если ЗначениеЗаполнено(Параметры["/DB_User"]) Тогда 
			ПараметрыСУБД.Вставить("ПользовательСУБД", Параметры["/DB_User"]);
			Лог.Информация("ПользовательСУБД: " + ПараметрыСУБД.ПользовательСУБД);
		Иначе
			ПараметрыСУБД.Вставить("ПользовательСУБД", "");
		КонецЕсли;		

		Если ЗначениеЗаполнено(Параметры["/DB_Password"]) Тогда 
			ПараметрыСУБД.Вставить("ПарольСУБД", Параметры["/DB_Password"]);
			Лог.Информация("ПарольСУБД: ***");
		Иначе
			ПараметрыСУБД.Вставить("ПарольСУБД", "");
		КонецЕсли;

		Если ЗначениеЗаполнено(Параметры["/DB_Name"]) Тогда 
			ПараметрыСУБД.Вставить("ИмяБД", Параметры["/DB_Name"]);
		Иначе
		...
		Если ЗначениеЗаполнено(Параметры["/CN"]) Тогда 
			АвторизацияВКластере.Вставить("Имя", Параметры["/CN"]);
			Лог.Информация("Администратор кластера: " + АвторизацияВКластере.Имя);
		Иначе
			АвторизацияВКластере.Вставить("Имя", "");
		КонецЕсли;
		Если ЗначениеЗаполнено(Параметры["/CP"]) Тогда 
			АвторизацияВКластере.Вставить("Пароль", Параметры["/CP"]);
		...
		Попытка
			...
			Лог.Информация("Создание ИБ завершено");
		Исключение												   
			Лог.Ошибка("Произошла ошибка при создание ИБ на сервере " + Конфигуратор.ВыводКоманды());
			КодВозврата = 70;
			Возврат;
		КонецПопытки;

	Иначе
		Лог.Ошибка("Не заполнен параметр ""/F"" или ""/S"" - для создания файловой или серверной базы");
		КодВозврата = 69;
		Возврат;
	КонецЕсли;
...
// oscript "c:\Projects\uni_script\uni_script.os" /DeleteInfoBase /S alba\zmysupertest /DeleteMode 1
Процедура УдалитьИнформационнуюБазу()

	// ToDO Не удаляет на сервере MS SQL
	Если ЗначениеЗаполнено(Параметры["/F"]) Тогда
		Каталог = Параметры["/F"];
		Если НЕ ФайлСуществует(Каталог) Тогда
			Лог.Ошибка("Каталог <" + Каталог + "> с файловой информационной базой не существует");
			КодВозврата = 70;
			Возврат;
		КонецЕсли;
		...
		Исключение
			Лог.Ошибка("Ошибка удаления файловой информационной базы " + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КодВозврата = 70;
			Возврат;
		КонецПопытки;
	...
	ИначеЕсли ЗначениеЗаполнено(Параметры["/S"]) Тогда
		...
		ИмяСервера					= Параметры["ИмяСервера"];
		Лог.Информация("Имя сервера: " + ИмяСервера);
		ИмяБазы						= Параметры["ИмяБазы"];
		Лог.Информация("Имя удаляемой ИБ: " + ИмяБазы);
		ИмяАдминистратораИБ			= Параметры["/N"];
		Если НЕ ЗначениеЗаполнено(ИмяАдминистратораИБ) Тогда
			ИмяАдминистратораИБ = "";
		КонецЕсли;
		ПарольАдминистратораИБ 		= Параметры["/P"];
		Если НЕ ЗначениеЗаполнено(ПарольАдминистратораИБ) Тогда
		...
		КонецЕсли;
		РежимУдаления = 1;
		Если ЗначениеЗаполнено(Параметры["/DeleteMode"]) Тогда
			
			Попытка
				РежимУдаления = Число(СокрЛП(Параметры["/DeleteMode"]));
			Исключение
				Лог.Ошибка("Ошибка преобразования параметра /DeleteMode в число. /DeleteMode должен быть равен числу 0, 1 или 2");
				КодВозврата = 70;
				Возврат;
			КонецПопытки;

			Если НЕ (РежимУдаления = 0 ИЛИ РежимУдаления = 1 ИЛИ РежимУдаления = 2) Тогда
				Лог.Ошибка("Параметр /DeleteMode должен быть равен числу 0, 1 или 2. А он равен <" + Строка(РежимУдаления) + ">");
				КодВозврата = 70;
				Возврат;
			КонецЕслИ;
		КонецЕсли;
		...
		Попытка
			...
			Для Каждого Кластер из Кластеры Цикл				
				...
				Для Каждого Процесс из Процессы Цикл
					...
					Для Каждого База из Базы Цикл										
						Если ВРег(СокрЛП(База.Name)) = ВРег(СокрЛП(ИмяБазы)) Тогда						
							ИБНайденна = Истина;
							
							Лог.Информация("Информационная база <" + База.Name + "> найдена");
							Лог.Информация("Имя сервера БД: " + База.dbServerName);
							Лог.Информация("Имя удаляемой базы БД: " + База.dbName);
						
							РабПроц.DropInfoBase(База, 1); 
	
							Прервать;
						КонецЕсли;
					КонецЦикла;

					Если НЕ ИБНайденна Тогда
						Лог.Ошибка("ИБ не найдена!");
						КодВозврата = 3;
						ЗавершитьРаботу(КодВозврата);
						Возврат;
		...
		ВыполнитьСборкуМусора();
	Иначе
		Лог.Ошибка("Не заполнен параметр ""/F"" или ""/S"" - для удаления файловой или серверной базы");
		КодВозврата = 70;
		Возврат;
	КонецЕсли;

	Лог.Информация("Удаление информационной базы выполнено");
...
Функция ПрочитатьСекциюINIФайла(Знач ИмяФайла, Знач ИмяСекции)
	...
	ТД 			= Новый ТекстовыйДокумент();
	ТД.Прочитать(ИмяФайла); // Соломахин - убрана кодировка UTF-8 из-за проблемы отображения кириллицы. Возможно нужно будет провести настройки Windows на Альбе.
	СекцияНайдена = Ложь;
...
Процедура ПроверкаКонфигурации()
	...
	ПараметрыЗапуска.Добавить("/UseHwLicenses-");
...
Процедура СформироватьФайлНастроекVanessaAutomation()
	...
	СтрокаНастроекVanessa = "{
		...
		|		""РазличатьBrokenИFailedПоКлючевомуСловуТогда"": false,
		|		""ЗапрещатьПерезаписыватьПеременные"": false,
		|		""ОбновлятьТаблицыФормыПриПолученииЗначения"": false,
		|		""ПолучатьМакетыСПомощьюРасширенияVAExtension"": false,
		|		""ВыполнятьSleepЧерезPing"": false,
		|		""ТаймаутДляАсинхронныхШагов"": 10,
		|		""КоличествоСекундПоискаОкна"": 10,
		|		""КоличествоПопытокВыполненияДействия"": 1,
...
Процедура СоздатьШаблонПочтовойРассылкиОбновления()
	...
	ВерсияИзНастроек			= Настройки.Получить("VERSION");
	СписокВерсийДляОбновления	= Настройки.Получить("VERSION_UPDATES");
	БагВерсииОбновлений			= Настройки.Получить("VERSION_BUG_UPDATES");
	ВерсияСервераЛицензирования = Настройки.Получить("VERSION_LICENSE_SERVER");
	ИмяПрограммы 			 	= Настройки.Получить("PROGRAM");
	ИдентефикаторПрограммы		= Настройки.Получить("SITE_ID_PROGRAM");
	...
	Попытка
		...
		СписокСсылокПолучен = Ложь;
		Макет = Соединение.ПолучитьОбщийМакет(ИмяМакета);		
		Если Соединение.Метаданные.ОбщиеМодули.Найти("ОбновлениеИнформационнойБазыКлиентПереопределяемый") <> Неопределено Тогда
			Попытка
				Лог.Информация("Попытаемся получить ссылки из общего модуля - <ОбновлениеИнформационнойБазыКлиентПереопределяемый>");
				СписокСсылок 	= Соединение.ОбновлениеИнформационнойБазыКлиентПереопределяемый.СтруктураОписанияОбновлений();
				СписокСсылокПолучен = Истина;
			Исключение
			КонецПопытки;
		КонецЕсли;

		Если СписокСсылокПолучен = Ложь Тогда		
			Лог.Информация("Не удалось получить ссылки из общего модуля, поэтому получаем ссылки из файла settings.ini");
			СписокСсылок = ПрочитатьСекциюINIФайла(Settings, SectionURL);
		КонецЕсли;
		...
		Для Индекс = 1 По Области.ВысотаТаблицы Цикл
			...
			Если НЕ ПустаяСтрока(Раздел) Тогда
				Если Раздел = "Внимание!" Тогда
					...
					СтрокаПревью = "";
					СекцияПерьвюВидео = ШаблоныHTML.preview;
					...
					Если Области.Область(Индекс, 2, Индекс, 2).Гиперссылка = Истина Тогда
						...
						ПараметрыОбработки = Новый Структура("link, link_description", НайденныйURL, Текст);
						УстановитьПараметрыHTML(СекцияСсылокHTML, ПараметрыОбработки);
						
						Если СтрНайти(НРег(НайденныйURL), "youtu") > 0 Тогда
							ПараметрыОбработки = Новый Структура("link_video, link_preview");
							ПараметрыОбработки.link_video = НайденныйURL;
							ПараметрыОбработки.link_preview = "https://softonit.ru/upload/news/" + Сред(НайденныйURL, СтрНайти(НайденныйURL, "/", НаправлениеПоиска.СКонца) + 1) + ".jpg";
							УстановитьПараметрыHTML(СекцияПерьвюВидео, ПараметрыОбработки);
							СтрокаПревью = СтрокаПревью + СекцияПерьвюВидео;
						КонецЕсли;

						Если СтрНайти(НРег(НайденныйURL), ".gif") > 0 Тогда
							ПараметрыОбработки = Новый Структура("link_video, link_preview");
							ПараметрыОбработки.link_video = НайденныйURL;
							ПараметрыОбработки.link_preview = "https://softonit.ru/upload/news/" + Сред(НайденныйURL, СтрНайти(НайденныйURL, "/", НаправлениеПоиска.СКонца) + 1) + ".jpg";
							УстановитьПараметрыHTML(СекцияПерьвюВидео, ПараметрыОбработки);
						...
						Текст = СекцияСсылокHTML + СтрокаПревью;
		...
		МассивВерсийДляОбновления = РазложитьСтрокуВМассивПодстрок(СписокВерсийДляОбновления);
		МассивВерсийСОшибками	  = РазложитьСтрокуВМассивПодстрок(БагВерсииОбновлений, , , Истина);
		
		Если МассивВерсийДляОбновления.Количество() > 0 Тогда
			СтрокаВерсийОбновления = "";
			Для Каждого СтрокаВерсия Из МассивВерсийДляОбновления Цикл	
				НайденаБагВерсия = МассивВерсийСОшибками.Найти(СокрЛП(СтрокаВерсия));
				Если НайденаБагВерсия <> Неопределено Тогда
					Продолжить;
				КонецЕсли;
...
// oscript "D:\Projects\uni_script\uni_script\uni_script.os" /SendMailOnApiSendSay /UPath G:\mailing.html /KeyAPI 18GL7PRbjE731P1FUIZ5n_.... /ID 75 /Name "Тест API ШАБЛОН2" /IDSegment "pl16995" /Settings D:\Projects\IT\uit\settings.ini /Section SETTINGS 
Процедура ОтправитьШаблонРассылкиВSendSay()
	...
	Структура.Вставить("subject", "Обновление " + ИмяПрограммы + " версия " + ВерсияИзНастроек);
...
Функция ОбернутьВДвойныеКавычки(Стр)
	...
	Результат = Стр;
	Если Лев(Стр, 1) <> Символ(34) Тогда
		Результат = Символ(34) + Результат;
	КонецЕсли;
	Если Прав(Стр, 1) <> Символ(34) Тогда
...
// oscript "c:\Projects\uni_script\uni_script.os" /DumpExternalDataProcessorOrReportToFiles /FileName "c:\Projects\word1c\Печать по шаблонам Word в 1С.epf" /Catalog "c:\Temp\1\"
Процедура ВыгрузитьВнешнийОтчетИлиОбработкуВФайлы()

	Если Не ЗначениеЗаполнено(Параметры["/FileName"]) Тогда
		Лог.Ошибка("Имя файла с обработкой не задано");
		КодВозврата = 158;
		Возврат;
	КонецЕсли;	
	ИмяФайла = Параметры["/FileName"];
	Если НЕ ФайлСуществует(ИмяФайла) Тогда
		Лог.Ошибка("Имя файла с обработкой не существует");
		КодВозврата = 159;
		Возврат;
	КонецЕсли;

	Если Не ЗначениеЗаполнено(Параметры["/Catalog"]) Тогда
		Лог.Ошибка("Каталог для сохранения внешнего отчета/обработки не задан");
		КодВозврата = 160;
		Возврат;
	КонецЕсли;
	ПутьСохранения = Параметры["/Catalog"];
	// Если НЕ ФайлСуществует(Каталог) Тогда
	// 	Созда
	// КонецЕсли;
	...
	ПараметрыЗапуска 	= Конфигуратор.ПолучитьПараметрыЗапуска();
	ИмяКоманды = СтрШаблон("/DumpExternalDataProcessorOrReportToFiles %1 %2 %3",
		ОбернутьВДвойныеКавычки(ПутьСохранения),
		ОбернутьВДвойныеКавычки(ИмяФайла),
		ОбернутьВДвойныеКавычки(ИмяВременногоФайла)
	);
	ПараметрыЗапуска.Добавить(ИмяКоманды);	
	Лог.Информация("Начало сохранения отчета/обоработки в XML...");
	Лог.Информация(ИмяКоманды);
	Попытка	
	    Конфигуратор.ВыполнитьКоманду(ПараметрыЗапуска);
	Исключение												   
		Лог.Ошибка("Произошла ошибка при сохранении отчета/обоработки в XML " + Конфигуратор.ВыводКоманды());
		КодВозврата = 6;
	КонецПопытки;

	Если ФайлСуществует(ИмяВременногоФайла) Тогда
		ТекстовыйДокумент = Новый ТекстовыйДокумент;
		ТекстовыйДокумент.Прочитать(ИмяВременногоФайла);
		Лог.Информация(ТекстовыйДокумент.ПолучитьТекст());
		УдалитьФайлы(ИмяВременногоФайла);
	КонецЕсли;
...
Процедура ВыполнитьТестированиеVanessaAutomation()
	...
	Если Не ЗначениеЗаполнено(ФайлVA) Тогда
		Лог.Ошибка("Имя файла с обработкой Vanessa Automation.epf не задано (/FileName)");
		КодВозврата = 158;
		Возврат;
	КонецЕсли;
	Если НЕ ФайлСуществует(ФайлVA) Тогда
		Лог.Ошибка("Файл с обработкой Vanessa Automation.epf не существует (/FileName)");
		КодВозврата = 158;
		Возврат;
	КонецЕсли;
	WorkspaceRoot = Параметры["/WorkspaceRoot"];
	Если Не ЗначениеЗаполнено(ФайлVA) Тогда
		Лог.Ошибка("Имя каталога с тестами не задано (/WorkspaceRoot)");
		КодВозврата = 158;
		Возврат;
	КонецЕсли;
	VAParams = Параметры["/VAParams"];
	Если Не ЗначениеЗаполнено(ФайлVA) Тогда
		Лог.Ошибка("Имя файла с параметрами тестов не задано (/VAParams)");
		КодВозврата = 158;
		Возврат;
	КонецЕсли;	
	...
	Попытка	
	    Результат = Конфигуратор.ВыполнитьКоманду(ДополнительныеПараметры);		
	Исключение												   
		Лог.Ошибка("Произошла ошибка при запуске тестов Vanessa Automation " + Конфигуратор.ВыводКоманды());
		КодВозврата = 6;
	КонецПопытки;
