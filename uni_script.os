...
Процедура ВключитьОтладкуБазы()
	...
	ВерсияПлатформы = Параметры["/VerPL"];
	Если Не ЗначениеЗаполнено(ВерсияПлатформы) Тогда
		Лог.Ошибка("Не заполнен параметр ""/VerPL"", который используется для получения пути платформы.");
		КодВозврата = 108;
	КонецЕсли;
	...
	Для Каждого Параметр Из мПараметрыЗапуска Цикл
		СтрокаЗапуска = СтрокаЗапуска + " " + Параметр;
	КонецЦикла;
	Сообщить("Итоговая строка запуска:" + " " + СтрокаЗапуска);
