...
Процедура ЗадатьНачальныеНастройки()	
	...
	юсГитРепозиторий 	= Новый ГитРепозиторий();
...
Функция ЗаполнитьПараметры()
	...
	юсПарсер.ДобавитьИменованныйПараметр("/ConfigurationRepositoryF");	// Репозиторий путь
	юсПарсер.ДобавитьИменованныйПараметр("/ConfigurationRepositoryN");	// Репозиторий имя пользователя
	юсПарсер.ДобавитьИменованныйПараметр("/ConfigurationRepositoryP");	// Репозиторий пароль
	юсПарсер.ДобавитьИменованныйПараметр("/Legal");						// Подтверждать легальность обновления
	юсПарсер.ДобавитьИменованныйПараметр("/NeedVerPL");					// Необходимая версия ПО
	юсПарсер.ДобавитьИменованныйПараметр("/Maket");						// Имя макета
	юсПарсер.ДобавитьИменованныйПараметр("/Catalog");					// Имя папки
	юсПарсер.ДобавитьИменованныйПараметр("/Branch");					// Ветка GIT
	юсПарсер.ДобавитьИменованныйПараметр("/Project");					// Проект
	юсПарсер.ДобавитьИменованныйПараметр("/ConfigurationFiles");		// 
	юсПарсер.ДобавитьИменованныйПараметр("/WorkspaceLocation");			// 
...
//				- /Format формат выгрузки (Plain)
	...
	Если юсПараметры["/Format"] <> Неопределено Тогда
		ПараметрыЗапуска.Добавить("-Format " + юсПараметры["/Format"]);
	КонецЕсли;
	юсСообщить("INFO ", "Начало выгрузки в файлы");
	Попытка
	    юсКонфигуратор.ВыполнитьКоманду(ПараметрыЗапуска);
		юсСообщить("INFO ", "Выгрузка завершена");
	Исключение												   
		юсСообщить("ERROR", "Произошла ошибка при выгрузке  в файлы " + юсКонфигуратор.ВыводКоманды());		
	КонецПопытки;	
...
Процедура СоздатьДистрибутивИзХранилища()	
	...
	НомерШага = 1;
	ПараметрыЗапуска1 			= юсКонфигуратор.ПолучитьПараметрыЗапуска();
	ПараметрыЗапуска1.Добавить("/ConfigurationRepositoryUpdateCfg -revised -force");
	ПараметрыЗапуска1.Добавить("/ConfigurationRepositoryF """ + юсПараметры["/ConfigurationRepositoryF"] + """");	
	ПараметрыЗапуска1.Добавить("/ConfigurationRepositoryN """ + юсПараметры["/ConfigurationRepositoryN"] + """");
	Если ЗначениеЗаполнено(юсПараметры["/ConfigurationRepositoryP"]) Тогда
		ПараметрыЗапуска1.Добавить("/ConfigurationRepositoryP """ + юсПараметры["/ConfigurationRepositoryP"] + """");
	КонецЕсли;
		
	юсСообщить("INFO ", "ШАГ " + Строка(НомерШага)); 
	юсСообщить("INFO ", "Получаем конфигурацию из хранилища");
	Попытка
	    юсКонфигуратор.ВыполнитьКоманду(ПараметрыЗапуска1);
	...
	ПараметрыЗапуска2 			= юсКонфигуратор.ПолучитьПараметрыЗапуска();
	ПараметрыЗапуска2.Добавить("/UpdateDBCfg");
	
	юсСообщить("INFO ", "ШАГ " + Строка(НомерШага));
	юсСообщить("INFO ", "Обновление конфигурации базы данных");
	Попытка
	    юсКонфигуратор.ВыполнитьКоманду(ПараметрыЗапуска2);
	...
	ПараметрыЗапуска3 			= юсКонфигуратор.ПолучитьПараметрыЗапуска();
	ПараметрыЗапуска3.Добавить("/CreateDistributionFiles -cffile """ + мКаталогВерсии + "1Cv8.cf" + """");
	Если мМассивВерсий.Количество() > 0 Тогда
	...
	Попытка
		юсКонфигуратор.ВыполнитьКоманду(ПараметрыЗапуска3);
	...
	ПараметрыЗапуска4 			= юсКонфигуратор.ПолучитьПараметрыЗапуска();
	...
	Попытка
		юсКонфигуратор.ВыполнитьКоманду(ПараметрыЗапуска4);
	...
	Если мМассивВерсий.Количество() > 0 Тогда
		ПараметрыЗапуска5 			= юсКонфигуратор.ПолучитьПараметрыЗапуска();
		...
		Попытка
			юсКонфигуратор.ВыполнитьКоманду(ПараметрыЗапуска5);
	...
	//юсСообщить("INFO ", "ВНИМАНИЕ! Далее сделайте архив поставки вручную из конфигуратора!");
...
Процедура ПолучитьОписаниеОбновленияИзМакета(ФайлHTML = Неопределено)
	...
	Если НЕ ЗначениеЗаполнено(юсПараметры["/Maket"]) Тогда
		ИмяМакета = "ОписаниеИзмененийСистемы";
	Иначе
		ИмяМакета = юсПараметры["/Maket"];
	КонецЕсли;
	...
	Попытка
		...
		Если Файл.Существует() Тогда
			...
			юсСообщить("INFO ", "Успешное замена URL в обновлении.");
...
//				- /ConfigurationRepositoryP пароль
	...
	юсСообщить("INFO ", "Успешное завершение обновления конфигурации из хранилища.");
...
//				- /Catalog <путь> (Путь к каталогу конфигурации)
	...
	Если НЕ ЗначениеЗаполнено(юсПараметры["/Catalog"]) Тогда
		юсСообщить("ERROR", "Не заполнен параметр ""/Catalog"" - путь каталогу с файлами конфигурации!");
		Возврат;
	КонецЕсли;

	Каталог = юсПараметры["/Catalog"];
	...
	юсСообщить("INFO ", "ШАГ " + Строка(НомерШага)); 
	юсСообщить("INFO ", "Загружаем конфигурацию из файлов");
	Попытка
	    юсКонфигуратор.ВыполнитьКоманду(ПараметрыЗапуска);
		юсСообщить("INFO ", "Загрузка конфигурации завершена");
	Исключение												   
		юсСообщить("ERROR", "Произошла ошибка при загрузке конфигурации из файлов " + юсКонфигуратор.ВыводКоманды());		
		Возврат;
	КонецПопытки;
	НомерШага = НомерШага + 1;
	
	юсКонфигуратор.УстановитьКонтекст("/F" + юсПараметры["/F"], "", "");
	ПараметрыЗапуска2 	= юсКонфигуратор.ПолучитьПараметрыЗапуска();
	ПараметрыЗапуска2.Добавить("/UpdateDBCfg");
	
	юсСообщить("INFO ", "ШАГ " + Строка(НомерШага));
	юсСообщить("INFO ", "Обновление загруженной конфигурации базы данных");
	Попытка
	    юсКонфигуратор.ВыполнитьКоманду(ПараметрыЗапуска2);
		юсСообщить("INFO ", "Обновление загруженной конфигурации завершено");
	Исключение												   
		юсСообщить("ERROR", "Произошла ошибка при обновлении загруженной конфигурации базы данных " + юсКонфигуратор.ВыводКоманды());
		Возврат;
	КонецПопытки;
	НомерШага = НомерШага + 1;
	...
	юсСообщить("INFO ", "Успешное завершение загрузки конфигурации из файлов");
...
//				- /Branch <ветка> (ветка в GIT)
	...
	Если НЕ ЗначениеЗаполнено(юсПараметры["/Catalog"]) Тогда
		юсСообщить("ERROR", "Не заполнен параметр ""/Catalog"" - путь каталогу с репозитарием git!");
		Возврат;
	КонецЕсли;
	КаталогРепозитория = юсПараметры["/Catalog"];
	
	Если НЕ ЗначениеЗаполнено(юсПараметры["/Branch"]) Тогда
		Ветка = "master";
	Иначе
		Ветка = юсПараметры["/Branch"];
	КонецЕсли;	
	
	НомерШага = 1;	
	юсСообщить("INFO ", "ШАГ " + Строка(НомерШага)); 
	юсСообщить("INFO ", "Устанавливаем рабочий каталог");	
	юсГитРепозиторий.УстановитьРабочийКаталог(КаталогРепозитория);
	НомерШага = НомерШага + 1;
	
	Если НЕ юсГитРепозиторий.ЭтоРепозиторий() Тогда
		юсСообщить("ERROR", "Указанный каталог не является репозитарием git!");
		Возврат;		
	КонецЕсли;
	
	юсГитРепозиторий.УстановитьНастройку("core.quotePath", "true", РежимУстановкиНастроекGit.Локально);

	Если ВРег(юсГитРепозиторий.ПолучитьТекущуюВетку()) <> ВРег(СокрЛП(Ветка)) Тогда
		юсСообщить("INFO ", "ШАГ " + Строка(НомерШага)); 
		юсСообщить("INFO ", "Переходим в ветку " + Ветка);		
		юсГитРепозиторий.ПерейтиВВетку(Ветка);
		НомерШага = НомерШага + 1;
	КонецЕсли;
	
	юсСообщить("INFO ", "ШАГ " + Строка(НомерШага)); 
	юсСообщить("INFO ", "Получаем изменения из удаленного репозитория ветки " + Ветка);
	юсГитРепозиторий.Получить();
	НомерШага = НомерШага + 1;

	ТаблицаПодмодулей = юсГитРепозиторий.ПолучитьСостояниеПодмодулей();
	Если ТаблицаПодмодулей.Количество() > 0 Тогда
		юсСообщить("INFO ", "ШАГ " + Строка(НомерШага)); 
		юсСообщить("INFO ", "Обновляем подмодули ветки " + Ветка);
		юсГитРепозиторий.ОбновитьПодмодули(Истина, Истина);	
	КонецЕсли;
...
//				- /WorkspaceLocation (Обязательный параметр Директория рабочего простанства для запуска 1C:Enterprise Development Tools.)
	...
	Если НЕ ЗначениеЗаполнено(юсПараметры["/Project"]) Тогда
		юсСообщить("ERROR", "Не заполнен параметр ""/Project"" - директория, содержащая проект 1C:Enterprise Development Tools!");
		Возврат;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(юсПараметры["/ConfigurationFiles"]) Тогда
		юсСообщить("ERROR", "Не заполнен параметр ""/ConfigurationFiles"" - директория для экспорта файлов конфигурации платформы 1С:Предприятия!");
		Возврат;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(юсПараметры["/WorkspaceLocation"]) Тогда
		юсСообщить("ERROR", "Не заполнен параметр ""/WorkspaceLocation"" - директория рабочего простанства для запуска 1C:Enterprise Development Tools!");
		Возврат;
	КонецЕсли;
	
	Project 			= юсПараметры["/Project"];
	ConfigurationFiles 	= юсПараметры["/ConfigurationFiles"];
	WorkspaceLocation 	= юсПараметры["/WorkspaceLocation"];
	retCode = 0;
	
	НомерШага = 1;
	юсСообщить("INFO ", "ШАГ " + Строка(НомерШага)); 
	юсСообщить("INFO ", "Экспорт проекта 1C:Enterprise Development Tools в файлы конфигурации платформы 1С:Предприятия (XML)");
	...
	юсСообщить("INFO ", "Код возврата " + Строка(retCode)); 
	
КонецПроцедуры

Процедура СоздатьИнформационнуюБазу()

	Если НЕ ЗначениеЗаполнено(юсПараметры["/F"]) Тогда
		юсСообщить("ERROR", "Не заполнен параметр ""/F"" - директория, для создания ИБ!");
		Возврат;
	КонецЕсли;
	
	Каталог = юсПараметры["/F"];
	
	НомерШага = 1;
	юсСообщить("INFO ", "ШАГ " + Строка(НомерШага)); 
	юсСообщить("INFO ", "Создание ИБ в каталоге <" + Каталог + ">");
	Попытка
	    юсКонфигуратор.СоздатьФайловуюБазу(Каталог);
		юсСообщить("INFO ", "Создание ИБ завершено");
	Исключение												   
		юсСообщить("ERROR", "Произошла ошибка при загрузке конфигурации из файлов " + юсКонфигуратор.ВыводКоманды());
		Возврат;
	КонецПопытки;		
		
	юсСообщить("INFO ", "Успешное созданние конфигурации.");
