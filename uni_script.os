...
Процедура ЗапускVanessaAutomation()
	...
	Если ЗапускCoverage41C = ВРег("True") Тогда
		...
		АдресОтладки = Параметры["/URLDebug"];
		Если Не ЗначениеЗаполнено(АдресОтладки) Тогда
			Лог.Ошибка("Не заполнен параметр <URLDebug>");
			КодВозврата = 117;
		КонецЕсли;
...
Процедура ЗапуститьФайлОтладчика()
	...
	СтрокаЗапуска = ПутьКПлатформе + "\dbgs.exe" + "--addr=" + АдресОтладчика + "--port=" + ПортОтладки;
	...
	Попытка
		...
		Лог.Информация("Строка запуска: " + СтрокаЗапуска);
